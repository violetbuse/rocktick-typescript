...
import { Tenant, TenantCreateParams, TenantUpdateParams, Tenants } from './resources/tenants';
import { Verify, VerifyRetrieveResponse } from './resources/verify';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['ROCKTICK_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['ROCKTICK_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ROCKTICK_LOG'] or 'warn' if it isn't set.
   */
...
export class Rocktick {
   ...
   * @param {string} [opts.baseURL=process.env['ROCKTICK_BASE_URL'] ?? https://api.example.com] - Override the default base URL for the API.
  ...
  constructor({
    ...
    if (apiKey === undefined) {
      throw new Errors.RocktickError(
        "The ROCKTICK_API_KEY environment variable is missing or empty; either provide it, or instantiate the Rocktick client with an apiKey option, like new Rocktick({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Rocktick.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ROCKTICK_LOG'), "process.env['ROCKTICK_LOG']", this) ??
      defaultLogLevel;
  ...
  static Rocktick = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static RocktickError = Errors.RocktickError;
...
Rocktick.Cron = Cron;
Rocktick.Executions = Executions;
Rocktick.Jobs = Jobs;
Rocktick.Tenants = Tenants;
Rocktick.Verify = Verify;
...
export declare namespace Rocktick {
  ...
  export { Verify as Verify, type VerifyRetrieveResponse as VerifyRetrieveResponse };
}
